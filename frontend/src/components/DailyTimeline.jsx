...
export default function DailyTimeline({ history = [], forecast = [], startingSavings = 0, userId, language }) {
  const all = useMemo(() => {
  ...
  const handleClick = async (day) => {
    if (!day || day.isForecast || !userId || !day.date) return;
    setDrawer(day);
    setExplanation(null);
    setExplainLoading(true);
    try {
      const r = await api.explainDay(userId, day.date, language);
      setExplanation(r);
    } catch (err) {
      setExplanation({ error: err.message || "request_failed" });
    } finally {
        ...
        {all.map((d, i) => {
          ...
          return (
            <g
              ...
              style={{ cursor: d.isForecast ? "default" : "pointer" }}
            >
              {/* invisible hit-target full column for easier hover/click */}
        ...
        {hovered ? (
          <div>
            <div className="t-row">
              ...
              {!hovered.isForecast && <span className="footer-note">· click for AI explanation</span>}
            </div>
            <div className="t-row">
          ...
          <div className="footer-note">Hover any day · click history days for AI explanation</div>
        )}
      </div>
      ...
      {drawer && (
        <div className="day-drawer">
          <div className="day-drawer-head">
            <div>
              <strong>{shortDate(drawer.date)}</strong>{" "}
              <span className="chip">{DOW_LABELS[drawer.dow] || ""}</span>
            </div>
          ...
          <div className="day-drawer-stats">
            <div>
              ...
              <strong>{formatINR(drawer.earnings)}</strong>
            </div>
            <div>
              <span className="label">Spending</span>
              <strong>{formatINR(drawer.spending)}</strong>
            </div>
            <div>
              <span className="label">Net</span>
              <strong
                style={{ color: drawer.net >= 0 ? "var(--accent-2)" : "var(--danger)" }}
              >
                {drawer.net >= 0 ? "+" : ""}
                {formatINR(drawer.net)}
              </strong>
            </div>
          </div>

          <div className="day-drawer-explanation">
            <span className="briefing-tag">AI explanation</span>
            {explainLoading && (
              <p className="footer-note">Looking up day-of-week expectation, events, neighbours…</p>
            )}
            {explanation?.error && (
              <p className="footer-note error">{explanation.error}</p>
            )}
            {explanation?.text && (
              <>
                <p className="day-drawer-text">{explanation.text}</p>
                <span className="chip">
                  source: {explanation.source}
                  {explanation.snapshot?.anomaly?.kind &&
                    ` · ${explanation.snapshot.anomaly.kind} (z=${explanation.snapshot.anomaly.z}σ)`}
                </span>
              </>
            )}
          </div>
          ...
          {explanation?.snapshot && (
            <details className="day-drawer-source">
              <summary>Show source data the model saw</summary>
              <pre>{JSON.stringify(explanation.snapshot, null, 2)}</pre>
            </details>
          )}
        </div>
      )}
